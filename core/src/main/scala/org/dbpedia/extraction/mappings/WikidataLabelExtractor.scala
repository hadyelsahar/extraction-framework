          ...
          for (property <- node.getValueTriples.keys)
          {
            ...
            property match {
              case "http://www.w3.org/2000/01/rdf-schema#label" => {
                ...
                {
                    quads += new Quad(Language.apply(lang), DBpediaDatasets.Labels, subjectUri, labelProperty, labelsMap(lang), page.sourceUri, context.ontology.datatypes("xsd:string"))
                }
              }
